...
# Voiceover always on now — channel does 1 max-quality upload/day instead of 2,
# so every video gets the Hindi neural voiceover (Madhur voice).
...
# ---------- 4. GENERATE VOICEOVER + WORD-LEVEL TIMING (for synced captions) ----------
def generate_voiceover(text: str):
    ...
    Falls back to (audio_path, []) if word-boundary data isn't available —
    caller should fall back to static text in that case.
    """
    ...
    try:
        ...
        print(f"Voiceover word-timing generation failed, will use static text: {e}")

    return out_path, word_timings


def group_words_into_captions(word_timings: list, words_per_caption: int = 2) -> list:
    """Groups word timings into short 2-word caption chunks for readability
    (Hinglish reads better as short phrases than single words)."""
    captions = []
...
def build_video(bg1: Path, bg2: Path, music: Path, quote: str,
                 duration: int, clip_duration: int, voiceover: Path | None = None,
                 word_timings: list | None = None) -> Path:
    out_path = WORKDIR / "short.mp4"

    if word_timings:
        # Synced word-by-word captions (2 words at a time), timed to the voiceover —
        # the "viral Shorts" style where text pops up in sync with speech instead of
        # sitting static on screen the whole time.
        captions = group_words_into_captions(word_timings, words_per_caption=2)
        text_filters = []
        for cap in captions:
            safe_text = escape_drawtext(cap["text"])
            start = round(cap["start"], 2)
            end = round(max(cap["end"], start + 0.15), 2)  # avoid zero-length windows
            text_filters.append(
        ...
        drawtext_chain = ",".join(text_filters)
    else:
        # Fallback: static full-text overlay (used if voiceover/timing wasn't available)
...
def upload_to_youtube(video_path: Path, quote: str, caption: str, music_attribution: str) -> str:
    ...
    cat_hashtag = CONTENT_CATEGORIES.get(CONTENT_TYPE, {}).get("hashtag", "#studymotivation")
    hashtags = f"{cat_hashtag} #shorts #studentlife #examstress #discipline"
    tags = [CONTENT_TYPE.replace("_", " "), "shorts", "student life", "exam motivation", "discipline"]
...
# ---------- 6. PERFORMANCE LOG (Phase 6/17: lightweight CSV, committed back to repo) ----------
CONTENT_LOG_PATH = Path("content_log.csv")
CONTENT_LOG_HEADERS = [
    "content_id", "timestamp", "slot", "category", "hook_style",
    "title", "video_id", "upload_status", "voiceover", "duration_sec",
]

def log_content_entry(row: dict):
    import csv
    import uuid
    file_exists = CONTENT_LOG_PATH.exists()
    with open(CONTENT_LOG_PATH, "a", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CONTENT_LOG_HEADERS)
        if not file_exists:
            writer.writeheader()
        writer.writerow(row)
    print(f"Logged content entry: {row.get('content_id')}")
...
def main():
    ...
    print(f"Content category: {CONTENT_TYPE}")
    print(f"Hook style: {HOOK_STYLE}")
    print("Generating script + visual themes + music mood...")
    ...
    try:
        ...
        if VOICEOVER_ENABLED:
            print("Generating Hindi voiceover + word timings (Edge TTS)...")
            voice_text = quote.replace("\n", ". ")
            voiceover_path, word_timings = generate_voiceover(voice_text)
            voice_len = get_audio_duration(voiceover_path)
            duration = max(VIDEO_DURATION, int(voice_len) + 2)
            clip_duration = duration // 2
            print(f"Voiceover length: {voice_len:.1f}s -> video duration set to {duration}s")
            print(f"Word timings captured: {len(word_timings)} words")

        print("Building video...")
        video = build_video(bg1, bg2, music, quote, duration, clip_duration,
                             voiceover=voiceover_path, word_timings=word_timings)

        print("Uploading to YouTube...")
        video_id = upload_to_youtube(video, quote, data["caption"], attribution)
        ...
        log_content_entry({
            ...
            "voiceover": "yes" if VOICEOVER_ENABLED else "no",
            "duration_sec": duration,
        })
        if upload_status != "success":
            raise SystemExit("Pipeline failed — see logged status above.")
